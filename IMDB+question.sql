...
-- Manit Aslot , Sanika Behere, Srinivas seetharamaiah
...
SELECT 
	SUM(
			CASE 
				WHEN country like '%usa%' OR country like '%india%'
										THEN 1
                                        ELSE 0
				END
                ) AS usa_or_india_movies
...
round(sum(a.avg_rating * a.total_votes) / sum(a.total_votes), 2) AS actor_avg_rating
...
INNER JOIN movie m
ON r.movie_id = m.id
WHERE  r.category = 'actor' AND m.country LIKE '%India%'
...
INNER JOIN movie m
ON r.movie_id = m.id
WHERE  r.category = 'actress' AND m.country LIKE '%India%' AND m.languages LIKE '%Hindi%'
GROUP  BY r.name_id, n.NAME
